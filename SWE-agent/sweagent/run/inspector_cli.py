...
class TrajectoryInspectorApp(App):
    ...
    def _build_overview_stats(self):
        for traj in self.available_traj_paths:
            ...
            self.overview_stats[instance_id]["result"] = exit_status
            self.overview_stats[instance_id]["info"] = traj_info
            self.overview_stats[instance_id]["exit_status"] = exit_status
            self.overview_stats[instance_id]["api_calls"] = traj_info.get("model_stats", {}).get("api_calls", "?")
            self.overview_stats[instance_id]["cost"] = traj_info.get("model_stats", {}).get("instance_cost", "?")
